...
// Composant pour afficher une ligne de statistique avec valeurs domicile/extérieur
...
export default function MatchScore() {
  ...
  // État du formulaire principal (score + statistiques + prolongation/penalty)
  ...
  // Chargement des données du match au montage du composant
  useEffect(() => {
    api.get(`/matchs/${id}`)
  ...
  // Chargement des joueurs et des buts une fois le match récupéré
  useEffect(() => {
    if (!match) return;
  ...
  // Ajout d'un nouveau but via l'API
  const handleAddBut = async () => {
    ...
    try {
      ...
      // Réinitialisation du formulaire but après ajout
  ...
  // Suppression d'un but existant
  const handleDeleteBut = async (butId) => {
    await api.delete(`/buts/${butId}`);
    setButs(prev => prev.filter(b => b.id !== butId));
  };

  // Soumission du formulaire principal (score + prolongation/penalty si applicable)
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`/matchs/${id}/score`, {
        ...form,
        // Prolongation uniquement pour les phases éliminatoires
        prolongation: isKnockout ? prolongation : false,
        // Penalty uniquement si prolongation activée
        penalty: isKnockout && prolongation ? penalty : false,
      });
  ...
  // Affichage du chargement tant que le match n'est pas récupéré
  if (!match) {
    return <div className="flex items-center justify-center py-20 text-gray-500">Chargement...</div>;
  }

  // Vrai si le match est en phase éliminatoire (pas de groupe = pas de ligue)
  // => prolongation et penalty disponibles uniquement dans ce cas
  const isKnockout = match.type === 'COUPE' || match.phase === 'COUPE';

  const domNom = match.equipeDomicile?.nom || '?';
  const extNom = match.equipeExterieur?.nom || '?';

  // Liste des joueurs selon l'équipe sélectionnée pour le but
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl p-8">
        ...
        {/* Message de succès */}
        {success && (
          <div className="mb-6 bg-green-400/10 border border-green-400/30 text-green-400 px-4 py-3 rounded-lg text-sm">
            {success}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          {/* Section liste et ajout de buts */}
          <section className="border border-gray-800 rounded-xl p-5">
              ...
              <div className="flex flex-col gap-2 mb-4">
                {buts.map(b => (
                  <div key={b.id}
                    ...
                    {/* Badge type de but : CSC, Penalty ou Normal */}
            ...
            <div className="flex flex-col gap-3">
              ...
              {/* Sélection de l'équipe qui marque */}
              ...
              {/* Sélection du passeur (optionnel) */}
              <div className="relative">
          ...
          {/* Section prolongation et penalty — uniquement pour les phases éliminatoires (pas de ligue) */}
          {isKnockout && (
            <section className="border border-yellow-400/20 rounded-xl p-5">
              ...
              {prolongation && (
                <div className="flex flex-col gap-4">
                  ...
                  {/* Checkbox pour activer les tirs au but */}
